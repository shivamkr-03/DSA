...
void dfs(int u, int parent, vector<int> adj[], vector<bool>& visited, vector<int>& tin, vector<int>& low){
    ...
    for(int v: adj[u]){
        ...
        if(!visited[v]){
            ...
            // so UV is a bridge add it to the bridges and count bridge 
            if(tin[u] < low[v]){
                ++bridge;
...
int main(){
    ...
    for(int i = 0; i < n; ++ i){
        if(!visited[i]){
            dfs(i, -1, adj, visited, tin, low);
        }
    }
    cout << "no of bridges: " << bridge << endl;
    cout << "brigde edges:\n";
    for(auto i: bridges){
        cout << i[0] << "->" << i[1] << endl;
    }
